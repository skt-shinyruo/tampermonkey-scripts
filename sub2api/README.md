...
生成后的安装文件是 `sub2api-helper.user.js`。源码拆分在 `src/parts/` 下，修改源码后需要重新构建生成 userscript。发布版本由 GitHub Actions 构建并发布到 GitHub Pages，供 Greasy Fork 同步。
...
2. 上传或粘贴 GitHub Pages 上的 `sub2api-helper.user.js` 内容。
3. 在脚本设置中开启从 URL 同步，URL 填入上面的 GitHub Pages 构建产物地址。
4. 同步方式选择自动同步；如果需要立刻发布新版本，可以在 Greasy Fork 后台手动触发同步。

发布后，用户应从 Greasy Fork 安装脚本。GitHub Pages 地址只作为 Greasy Fork 的同步源和调试入口。
...
`sub2api-helper.user.js` 是生成产物。日常开发应修改 `src/parts/` 中的源码分片，再运行构建脚本生成最终 userscript。
...
构建 Greasy Fork 同步用的 GitHub Pages 产物：

```bash
SUB2API_UPDATE_URL="https://skt-shinyruo.github.io/tampermonkey-scripts/sub2api-helper.user.js" \
SUB2API_DOWNLOAD_URL="https://skt-shinyruo.github.io/tampermonkey-scripts/sub2api-helper.user.js" \
node sub2api/build-userscript.mjs --output=dist/sub2api-helper.user.js
```

仓库中的 `.github/workflows/sub2api-pages.yml` 会在 push 到 `main` 时自动运行测试、构建 `dist/sub2api-helper.user.js`，并部署到 GitHub Pages。workflow 使用 GitHub Actions 提供的 `GITHUB_REPOSITORY_OWNER` 和 `GITHUB_REPOSITORY` 自动计算 Pages URL，仓库改名后不需要手动改 workflow。
...
- 不要直接长期维护 `sub2api-helper.user.js`，否则下次构建会覆盖手工修改。
- 如果修改了 `src/parts/`，必须运行 `node sub2api/build-userscript.mjs`。
- 测试中包含构建一致性检查，会在生成文件过期时失败。
