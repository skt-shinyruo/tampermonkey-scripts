...
- 普通 `/usage` 与 `/admin/usage` 的日期范围、粒度，以及 `/usage`、`/admin/usage` 与 `/admin/accounts` 的每页数量必须独立存储和恢复；不要让一个页面的开关或保存值影响其他页面。
